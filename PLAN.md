...
**Thumbnails are files, not BLOBs.** `%LocalAppData%\ModelExplorer\thumbs\{hash[0..2]}\{hash}.png`, 256px. Generated by one dedicated STA render thread with an offscreen DX target; the queue is re-prioritized from the visible scroll viewport so what you're looking at renders first.
...
Offscreen render worker on its own STA thread, disk cache, viewport-driven priority queue, placeholder → fade-in. `VirtualizingWrapPanel` grid with a size slider and a list/grid toggle.
